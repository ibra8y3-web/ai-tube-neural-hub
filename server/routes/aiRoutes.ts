...
router.post("/chat", async (req, res) => {
  const { prompt, taskType } = req.body;
  try {
    const text = await aiService.generateChatResponse(prompt, taskType || "general");
    res.json({ text });
  } catch (error: any) {
    res.status(500).json({ error: error.message || "Failed to generate chat response" });
  }
...
router.post("/vision-chat", async (req, res) => {
  const { prompt, image } = req.body;
  try {
    // For vision, we specifically request a vision-capable model
    const bestModel = await aiService.getBestModelForTask("vision");
    const text = await aiService.callAiModel(bestModel, prompt, `Analyze this image: ${image}`); // Simplified for now, assuming base64 image handling in callAiModel
    res.json({ text });
  } catch (error: any) {
