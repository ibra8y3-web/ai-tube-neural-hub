...
import { ApiKeysManager } from "./components/features/ApiKeysManager";
import { 
  SmartChat, 
  LogoGeneratorLab, 
  RepairLab, 
...
import { DataSingularity, SwarmLab, OmniBridge, MarketTrends, APIArchitect, RegexMaster, LogAnalyzer, CSSPlayground, GitSuggest, LifePlanner, RecipeAlchemist, GiftFinder, StoryWeaver, TravelBuddy, SecurityVault, SQLOptimizer, SEOMaster, CVBuilder, NodeManager, PortWarden, BashWizard, JSONTransformer, PromptEngineer, ColorHarmonizer, ProjectArchitect, InnovationEngine, CodeTransformer, SnippetLibrary, DockerMaster, ReadmeGen, GhostData, LogicSolver, BrandNamer, UniversalTranslator, CodeArchitectTool, ContentAlchemist, DreamInterpreter, MoodCompanion, SmartMealPlanner, FrontendAgent, SocialTaskManager } from "./components/tubes/NewTubes";
import { OmniTranslator } from "./components/features/OmniTranslator";
...
export default function App() {
  ...
  const categoryTabs: { [key: string]: any[] } = {
    ...
    tubes: [
      ...
      { id: 'security_vault', icon: ShieldCheck, label: lang === 'ar' ? 'فك التشفير' : 'Decryptor', badge: 'PRO', badgeColor: 'bg-red-600 text-white' },
      { id: 'sql_optimizer', icon: Database, label: lang === 'ar' ? 'محسن SQL' : 'SQL Optimizer', badge: 'DEV', badgeColor: 'bg-emerald-600 text-white' },
      { id: 'seo_master', icon: SearchCode, label: lang === 'ar' ? 'خبير السيو' : 'SEO Master', badge: 'USER', badgeColor: 'bg-blue-600 text-white' },
      { id: 'social_task', icon: LayoutDashboard, label: lang === 'ar' ? 'المهام والتواصل' : 'Social Tasks', badge: 'NEW', badgeColor: 'bg-blue-600 text-white' },
      { id: 'cv_builder', icon: Briefcase, label: lang === 'ar' ? 'منشئ CV' : 'CV Builder', badge: 'USER', badgeColor: 'bg-purple-600 text-white' },
      { id: 'node_manager', icon: Server, label: lang === 'ar' ? 'مدير العقد' : 'Nodes', badge: 'LIVE', badgeColor: 'bg-zinc-600 text-white' },
      { id: 'univ_trans', icon: Languages, label: lang === 'ar' ? 'المترجم العالمي' : 'Translator', badge: 'NEW', badgeColor: 'bg-emerald-600 text-white' },
      { id: 'dream_interp', icon: Sparkles, label: lang === 'ar' ? 'مفسر الأحلام' : 'Dreams', badge: 'USER', badgeColor: 'bg-indigo-600 text-white' },
      { id: 'mood_comp', icon: Activity, label: lang === 'ar' ? 'رفيق المزاج' : 'Vibe', badge: 'USER', badgeColor: 'bg-orange-600 text-white' },
      { id: 'meal_plan', icon: Utensils, label: lang === 'ar' ? 'مخطط الوجبات' : 'Meals', badge: 'USER', badgeColor: 'bg-lime-600 text-white' },
      { id: 'code_arch', icon: TerminalSquare, label: lang === 'ar' ? 'مهندس الأكواد' : 'Code Arch', badge: 'DEV', badgeColor: 'bg-blue-600 text-white' },
      { id: 'cont_alch', icon: PenTool, label: lang === 'ar' ? 'خيميائي المحتوى' : 'Content', badge: 'CRE', badgeColor: 'bg-pink-600 text-white' },
      { id: 'docker_master', icon: Ship, label: lang === 'ar' ? 'خبير دوكر' : 'Docker', badge: 'OPS', badgeColor: 'bg-blue-600 text-white' },
      { id: 'readme_wiz', icon: FileText, label: lang === 'ar' ? 'صانع README' : 'Readme', badge: 'DOC', badgeColor: 'bg-zinc-700 text-white' },
      { id: 'ghost_data', icon: Ghost, label: lang === 'ar' ? 'بيانات وهمية' : 'Ghost Data', badge: 'TEST', badgeColor: 'bg-purple-600 text-white' },
      { id: 'logic_solver', icon: Cpu, label: lang === 'ar' ? 'محلل المنطق' : 'Logic Solver', badge: 'ALGO', badgeColor: 'bg-emerald-600 text-white' },
      { id: 'brand_namer', icon: Fingerprint, label: lang === 'ar' ? 'مبتكر الأسماء' : 'Naming', badge: 'BRAND', badgeColor: 'bg-pink-600 text-white' },
      { id: 'project_architect', icon: FolderTree, label: lang === 'ar' ? 'مهندس المشاريع' : 'Architect', badge: 'PRO', badgeColor: 'bg-blue-700 text-white' },
      { id: 'innov_engine', icon: Lightbulb, label: lang === 'ar' ? 'محرك الابتكار' : 'Innovation', badge: 'NEW', badgeColor: 'bg-amber-600 text-white' },
      { id: 'code_trans', icon: Code2, label: lang === 'ar' ? 'محول الأكواد' : 'Code Trans', badge: 'AI', badgeColor: 'bg-emerald-700 text-white' },
      { id: 'snippet_lib', icon: BookMarked, label: lang === 'ar' ? 'مكتبة الأكواد' : 'Snippets', badge: 'SAVE', badgeColor: 'bg-zinc-700 text-white' },
      { id: 'port_warden', icon: Radar, label: lang === 'ar' ? 'فاحص الشبكة' : 'Network', badge: 'SEC', badgeColor: 'bg-orange-600 text-white' },
      { id: 'bash_wizard', icon: Terminal, label: lang === 'ar' ? 'ساحر Bash' : 'Bash', badge: 'DEV', badgeColor: 'bg-zinc-500 text-white' },
      { id: 'json_trans', icon: FileJson, label: lang === 'ar' ? 'محول JSON' : 'JSON', badge: 'DATA', badgeColor: 'bg-yellow-600 text-black' },
      { id: 'prompt_eng', icon: Sparkles, label: lang === 'ar' ? 'مهندس برومبت' : 'Prompts', badge: 'AI', badgeColor: 'bg-indigo-600 text-white' },
      { id: 'color_harm', icon: Palette, label: lang === 'ar' ? 'ناسج ألوان' : 'Colors', badge: 'DES', badgeColor: 'bg-pink-600 text-white' },
    ...
    engineering: [
      { id: 'api_keys', icon: Terminal, label: lang === 'ar' ? 'مفاتيح CLI' : 'API Keys (CLI)', badge: 'CORE', badgeColor: 'bg-emerald-500 text-black' },
      { id: 'frontend_agent', icon: Monitor, label: lang === 'ar' ? 'وكيل الواجهات' : 'Frontend Agent', badge: 'NEW', badgeColor: 'bg-cyan-500 text-black' },
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                ...
                {activeSettingsTab === 'api_keys' && <ApiKeysManager lang={lang} />}
                {activeSettingsTab === 'frontend_agent' && <FrontendAgent lang={lang} />}
                ...
                {activeSettingsTab === 'security_vault' && <SecurityVault lang={lang} />}
                {activeSettingsTab === 'sql_optimizer' && <SQLOptimizer lang={lang} />}
                {activeSettingsTab === 'seo_master' && <SEOMaster lang={lang} />}
                {activeSettingsTab === 'cv_builder' && <CVBuilder lang={lang} />}
                {activeSettingsTab === 'node_manager' && <NodeManager lang={lang} />}
                {activeSettingsTab === 'social_task' && <SocialTaskManager lang={lang} />}
                {activeSettingsTab === 'univ_trans' && <OmniTranslator lang={lang} />}
                {activeSettingsTab === 'dream_interp' && <DreamInterpreter lang={lang} />}
                {activeSettingsTab === 'mood_comp' && <MoodCompanion lang={lang} />}
                {activeSettingsTab === 'meal_plan' && <SmartMealPlanner lang={lang} />}
                {activeSettingsTab === 'code_arch' && <CodeArchitectTool lang={lang} />}
                {activeSettingsTab === 'cont_alch' && <ContentAlchemist lang={lang} />}
                {activeSettingsTab === 'docker_master' && <DockerMaster lang={lang} />}
                {activeSettingsTab === 'readme_wiz' && <ReadmeGen lang={lang} />}
                {activeSettingsTab === 'ghost_data' && <GhostData lang={lang} />}
                {activeSettingsTab === 'logic_solver' && <LogicSolver lang={lang} />}
                {activeSettingsTab === 'brand_namer' && <BrandNamer lang={lang} />}
                {activeSettingsTab === 'project_architect' && <ProjectArchitect lang={lang} />}
                {activeSettingsTab === 'innov_engine' && <InnovationEngine lang={lang} />}
                {activeSettingsTab === 'code_trans' && <CodeTransformer lang={lang} />}
                {activeSettingsTab === 'snippet_lib' && <SnippetLibrary lang={lang} />}
                {activeSettingsTab === 'port_warden' && <PortWarden lang={lang} />}
                {activeSettingsTab === 'bash_wizard' && <BashWizard lang={lang} />}
                {activeSettingsTab === 'json_trans' && <JSONTransformer lang={lang} />}
                {activeSettingsTab === 'prompt_eng' && <PromptEngineer lang={lang} />}
                {activeSettingsTab === 'color_harm' && <ColorHarmonizer lang={lang} />}
